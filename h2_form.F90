...
FUNCTION H2_FORMATION_RATE(inside_outflow,GAS_TEMPERATURE,GRAIN_TEMPERATURE) RESULT(RATE)
   ...
   USE DEFINITIONS
   USE HEALPIX_TYPES
   USE GLOBAL_MODULE, ONLY: metallicity, g2d, g2d_outflow
   IMPLICIT NONE
...
!  Cazaux & Tielens (2004, ApJ, 604, 222)
   if (inside_outflow) then
      RATE=0.5D0*THERMAL_VELOCITY*(SILICATE_CROSS_SECTION*SILICATE_FORMATION_EFFICIENCY &
        & + GRAPHITE_CROSS_SECTION*GRAPHITE_FORMATION_EFFICIENCY)*STICKING_COEFFICIENT*METALLICITY*100./g2d_outflow
   else
      RATE=0.5D0*THERMAL_VELOCITY*(SILICATE_CROSS_SECTION*SILICATE_FORMATION_EFFICIENCY &
